...
class Bandwise2(object):
    ...
    def __call__(self, X, Y):
        ...
        # C = X.shape[-1]
        bwindex = []
        batchsize = X.shape[0]  #自己所添加
        for batch in range(batchsize):  #自己所添加
            for ch in range(C):
                x = torch.squeeze(X[batch,ch,:,:].data).cpu().numpy()  #自己所添加
                y = torch.squeeze(Y[batch,ch,:,:].data).cpu().numpy()  #自己所添加
...
def cal_mrae(X, Y):
    ...
    relative_error = abs_error / (Y + 1e-8)  # Adding a small value to avoid division by zero
    mrae = np.mean(relative_error)
    return mrae
...
def MSIQA2(X, Y):
    psnr = cal_imgpsnr(X, Y)
    ssim = cal_imgssim(X, Y)
    sam = cal_sam(X, Y)
    return psnr, ssim, sam
...
class SAMLoss2(nn.Module):
    ...
    def forward(self, img_base, img_out):
        ...
        sum2 = torch.sum(img_base * img_base, 1)
        sum3 = torch.sum(img_out * img_out, 1)
        t = torch.clip((sum1+self.eps)/(torch.sqrt(sum2)+self.eps)/(torch.sqrt(sum2)+self.eps),min=0,max=1)
        averangle = torch.mean(torch.acos(t))
        SAM = averangle * 180 / math.pi
        return SAM
