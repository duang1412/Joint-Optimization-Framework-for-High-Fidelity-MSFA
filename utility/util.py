...
def generate_SP(NF,three_D = True):
    M = round(NF / 2)
    ...
    for i in range(1, M + 1):
        for j in range(i, M + 1):
            G = np.mod(I * i + J * j, NF) + 1
            t = len(np.unique(G[0:NF, 0:NF]))

            if t == NF:
                G1 = G[0:NF, 0:NF]
                r, c = np.where(G1)
                z = np.array(G1[r,c])
                B = np.column_stack((r, c, z))
                D = np.min(pdist(B))
                distance[i - 1, j - 1] = D
            else:
                pass
    ma = np.max(distance)
    b,a = np.where(distance == ma)
    if NF == 16:
        pattern_num = 2
    elif NF == 12:
        pattern_num = 0
        ...
        pattern_num = 0
    G = np.mod(I * (a[pattern_num]+1) + J * (b[pattern_num]+1), NF)
    G_3D = np.zeros([NF,NF,NF])
    for i in range(NF):
        G_3D[i,:,:] = (G==i)
    if three_D :
        out = G_3D
