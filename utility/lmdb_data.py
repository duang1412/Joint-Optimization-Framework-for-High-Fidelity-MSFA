...
import lmdb
#import caffe
from utility.caffe_pb2 import Datum
...
def create_lmdb_train(
    ...
    # data = load(datadir + fns[0],'r')[matkey] # for ICVL; load = h5py.File
    data = load(datadir + fns[0])[matkey].T # for CAVE;load = scipy.io.loadmat
    print(data.shape)
    ...
    map_size = data.nbytes * len(fns) * 1.25  #default: map_size = data.nbytes * len(fns) * 2
...
#    import ipdb; ipdb.set_trace()
    ...
    with env.begin(write=True) as txn:
        ...
        for i, fn in enumerate(fns):
            # import ipdb; ipdb.set_trace()
            try:
                # X = load(datadir + fn)[matkey] # for ICVL; load = h5py.File
                X = load(datadir + fn)[matkey].T #for CAVE
            except:
...
#    import ipdb; ipdb.set_trace()
    ...
    with env.begin(write=True) as txn:
        ...
        for i, fn in enumerate(fns):
            ...
            for j in range(N):
                datum = Datum()   #caffe2.proto.caffe_pb2.Datum(),下载了一个caffe_pb2.py文件直接导入
                datum.channels = X.shape[1]
                datum.height = X.shape[2]
                datum.width = X.shape[3]
                datum.data = X[j].tobytes()
...
def create_icvl256_245():
    print('create icvl256_245...')
    datadir = 'F:\ICVL\selected_400_700nm/train/'
    fns = os.listdir(datadir)
    # fns = open('Data/ICVL/full_as_in_eccv_paper.txt').readlines()
    fns = [fn.split('.')[0]+'.mat' for fn in fns]
    
    create_lmdb_train(
        datadir, fns, 'D:\data/icvl_train/ICVL256_245', 'rad',
...
def create_icvl64_31():
    ...
    datadir = 'D:\duwenwang\data\icvl/train/'
    fns = os.listdir(datadir)
    fns = [fn.split('.')[0]+'.mat' for fn in fns]

    create_lmdb_train(
        datadir, fns, 'D:\duwenwang\datasets\hsi/ICVL64_31_100', 'rad',
        crop_sizes=(1024, 1024),
        scales=(1, 0.5, 0.25),
        ksizes=(31, 64, 64),
...
def create_harvard64_31():
    ...
    datadir = 'D:/data/Harvard/generate/'
    fns = os.listdir(datadir)
    #fns = [fn.split('.')[0]+'.mat' for fn in fns]

    create_lmdb_train(
        datadir, fns, 'D:/data/Harvard64_31_55_(1)', 'ref',
        crop_sizes=(1024, 1024),
        scales=(1, 0.5, 0.25),
        ksizes=(31, 64, 64),
        strides=[(31, 64, 64), (31, 32, 32), (31, 32, 32)],
...
def create_cave64_31():
    print('create harvard64_31...')
    datadir = 'D:\BaiduNetdiskDownload\dww\CAVE_mat/train/'
    fns = os.listdir(datadir)
    fns = [fn.split('.')[0]+'.mat' for fn in fns[0:20]]

    create_lmdb_train(
        datadir, fns, 'E:\dww/CAVE64_31_20(liu)', 'imgDouble',
        crop_sizes=None,
        scales=(1, 0.5, 0.25),
        ksizes=(31, 64, 64),
        strides=[(31, 64, 64), (31, 32, 32), (31, 32, 32)],
        load=scipy.io.loadmat, augment=True,
    )  #load=h5py.File

def create_cave128_31():
    print('create harvard128_31...')
    datadir = 'E:\dww\data\save_CAVE/train/'
    fns = os.listdir(datadir)
    fns = [fn.split('.')[0]+'.mat' for fn in fns[0:20]]

    create_lmdb_train(
        datadir, fns, 'E:\dww/CAVE128_31_20_test', 'orig',
        crop_sizes=None,
        scales=(1, 0.5, 0.25),
        ksizes=(31, 128, 128),
